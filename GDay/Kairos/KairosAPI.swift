...
struct KairosConfig {
    ...
    static let app_id = "5fe26197" //"5c15422d"
    static let app_key = "88ac9701b1d19907faae7b4cc6cc4b36" //"9bcf14f85b7403ece74ed9f2ef6ee408"
}
...
public class KairosAPI {
        ...
        let task = session.dataTask(with: request, completionHandler: { (data, response, error) -> Void in
            if let data = data {
                ...
                if let response = response as? HTTPURLResponse, 200...299 ~= response.statusCode, jsonErrors == nil {
                    ...
                    taskCallback(false, jsonResponse as AnyObject!, jsonErrors as AnyObject?)
    ...
    public func getError(errors: AnyObject?) -> String {
    ...
    public func request(method: String, data: Dictionary<String, Any>? = [:], httpTypeOverride: Any? = nil, callback: @escaping (AnyObject, String) -> ()) -> Void {
        ...
        self.send(url: url, data: data, httpType: httpType) { (ok, data, errors) in
            ...
            if ok == false {
                let error = self.getError(errors: errors)
